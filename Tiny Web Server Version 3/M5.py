            ...
            while True:
                ...
                with conn:
                    ...
                    if len(requrl) > 1 :
                        ...
                        uri = "/"
                    result = self.get_data(uri)
                    if result is None :
                        continue
                    else :
                        code, c_type, c_length, data = result
                    response = self.response_headers(code, c_type, c_length).encode() + data
                    conn.sendall(response)
                    conn.close()
    
    def get_data(self, uri):
        docroot = "/home/prem/Documents/IOS/M5_forever/"
        if uri == "/bin/":
            ...
            if pid:
                ...
                if info[0] == 0 :
                    ...
                    return 200, "text/plain", len("process timed out"), "process timed out".encode()
                else :
